...
keys = [
	...
	Key([mod], "n", lazy.spawn("flatpak run com.notepadqq.Notepadqq"), desc="launch notepadqq"),
	Key([mod], "f", lazy.spawn("thunar"), desc="launch thunar"),
	Key([mod], "F1", lazy.spawn(browser +  " https://docs.qtile.org/en/stable/"), desc="open qtile documentation"),
...
screens = [
	Screen(
		top=bar.Bar(
			[
				widget.TextBox("", fontsize=21, background=lightBlue), #
				...
				widget.Spacer(),
				widget.WidgetBox(text_closed="", text_open="" , widgets=[ #󰜬
					widget.Systray(),
				]),
				widget.Sep(),
				widget.Volume(fmt="󰕾 {}"),
				widget.Sep(),
				widget.Backlight(fmt="󰃠 {}", backlight_name='amdgpu_bl1', change_command='brightnessctl set {0}%', step=-10),
				widget.Sep(),
